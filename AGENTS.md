...
## Kingdom OS Mini App (miniapp/kingdom-os/)

- **URL:** https://jelfferyduran.github.io/kingdom-financial-services/kingdom-os/
- Sanitized public mirror of the KFS Admin OS (tailnet-only at kfs-os.tail258ffd.ts.net).
- `data.json` is AUTO-GENERATED by the KFS collector (`/opt/data/kfs-admin-os/collector.py` → `publish_miniapp()`), committed+ pushed only when changed. **Do not hand-edit.**
...
- **🔒 Locked-mode UI kit (`KOS_LOCKED`)** — reusable locked-mode components for any `?client=` view:
  ...
  - Read-only view components: `roCard({icon,title,body,meta})`, `roStat({lbl,num,sub,cls})`, `roKV(k,v)`, `roAccount({creditor,type,balance,limit,opened,note})`, `roList(items,renderItem)`, `roNotice(msg)` — display-only variants for dashboard/account/card data, no interactive controls.
  - Action gating: `PRIVILEGED_ACTIONS = ['send','edit','delete','approve']`, `isPrivileged(action)`, `gateAction(action,{label,icon,href,cls})`, `applyGates(root)`, `hideLocked(root)`, `guardClick(e)` (capture-phase click guard installed at boot).
  - Declarative DOM conventions (no JS per element): `data-privileged="send|edit|delete|approve"` → disabled when locked; `data-hide-when-locked` → `display:none` when locked.
- **🔗 Locked-mode navigation integration** — when `?client=<slug>` is in the URL, the WHOLE dashboard renders through the locked shell and the read-only kit: stats via `roStat`, sites/cases/roadmap sections wrapped in `roList`, every section card gets a 🔒 read-only chip, `#ask` (send) is gated + `guardClick`-blocked, and the full-OS link (`#full`) is hidden. The client param survives all internal navigation (hash, `pushState` via `KOS_STATE.navigate()`, popstate/back-forward re-sync), so deep links like `?client=anthony-duran#stats` land locked. No param / unknown slug → fully unlocked flow, byte-identical markup. External links (sites, case hubs) stay interactive in locked mode — they are read-only navigation.
- Test suites (jsdom at /tmp/kos-verify, `KOS_BASE` overrides target): `kos-lockedmode-test.js` (kit: locked/unlocked/inert/nav/idempotency/regression), `kos-urlstate-test.js` (routing state), `kos-navigation-test.js` (integration ACs: deep-link shell per client, all non-read-only actions hidden, param preserved across hash/pushState/popstate, unlocked flow intact).
- Launcher: pinned channel message + menu button in Noel's DM (Bot API `setChatMenuButton` web_app).
