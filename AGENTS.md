...
- **🔒 Locked-mode UI kit (`KOS_LOCKED`)** — reusable locked-mode components for any `?client=` view:
  ...
  - Test suite pattern: `kos-lockedmode-test.js` (jsdom at /tmp/kos-verify) — locked/unlocked/inert/nav/idempotency/regression checks, `KOS_BASE` overrides the target.
